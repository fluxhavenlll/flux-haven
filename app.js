...
function switchAdminTab(tab) {
    const panels = ['users', 'deposits', 'withdrawals', 'settings'];
    panels.forEach(p => {
        ...
        const btn = document.getElementById('adminTab' + (p === 'deposits' ? 'Dep' : p === 'withdrawals' ? 'Wd' : p.charAt(0).toUpperCase() + p.slice(1)));
        if (btn) {
