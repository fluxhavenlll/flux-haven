...
function renderWealthPlanCards(plans, existingPositions = []) {
    ...
    // All signed-in users can VIEW the plans. The Start Plan action itself
    // remains guarded by VIP and balance checks in startWealthPlan() and Supabase.
...
async function loadWealthCenterPage() {
    ...
    try {
        ...
        // Keep the page usable if the state endpoint has a transient problem.
        // The authoritative action checks remain in Supabase.
        renderWealthPlanCards(WEALTH_CENTER_PLANS, []);
        if (list) list.innerHTML = '<div class="glass-panel rounded-2xl p-6 text-center text-xs text-slate-500 border-white/5">No Wealth Center plans loaded yet. Tap refresh to try again.</div>';
    }
}
...
async function startWealthPlan(planCode) {
    ...
    if (!getActiveNewFeatureVipTier(currentUser)) {
        return showToast('An active VIP1–VIP6 plan is required to use the Wealth Center.');
    }
    ...
    if (Number(currentUser.balance || 0) < amount) {
        return showToast(`Insufficient available balance. You need at least ₦${fmt(amount)} to start ${plan.name}.`);
    }
...
async function loadLuckyHeartPage() {
    ...
    try {
        ...
        // Lucky Heart spins are earned from qualifying direct VIP activations;
        // the user does not need their own deposit/VIP just to receive a prize.
...
async function spinLuckyHeart() {
    ...
    try {
        ...
        const row = Array.isArray(data) ? data[0] : data;
        const reward = Number(row?.reward || 0);
        // Ten visual sectors are always shown. Only the three currently configured
        // server-side prize outcomes are allowed to drive the pointer.
        const prizeToSegment = {
            '₦699': 0,
            '₦1,999': 1,
            '₦2,999': 2
        };
        if (!Object.prototype.hasOwnProperty.call(prizeToSegment, row?.prize_label)) {
            throw new Error('Invalid Lucky Heart prize result');
        }
        const segmentIndex = prizeToSegment[row.prize_label];
...
window.addEventListener('pageshow', () => updatePwaInstallVisibility());
window.addEventListener('load', () => {
    updatePwaInstallVisibility();
    setTimeout(updatePwaInstallVisibility, 250);
    setTimeout(updatePwaInstallVisibility, 1500);
});
