...
const VIEWS = ['authPage', 'homePage', 'tasksPage', 'depositPage', 'withdrawPage', 'historyPage', 'profilePage', 'referralPage', 'adminPage'];
...
function navigateTo(page) {
    ...
    if (page !== 'profilePage' && page !== 'homePage' && page !== 'adminPage' && page !== 'historyPage' && page !== 'referralPage' && page !== 'tasksPage' &&
...
// ========================================================
const ALLOWED_WITHDRAWAL_AMOUNTS = [2000, 10000, 20000, 50000, 100000, 500000, 1000000, 3000000];
...
function prepareWithdrawalView() {
    ...
    if (day === 0) {
        ...
        if (status.open) {
            banner.classList.remove('border-red-500/20', 'border-amber-500/20');
            banner.classList.add('border-white/10');
            title.textContent = 'Withdrawals Open Today';
            title.className = 'text-sm font-bold text-cyan-400';
            desc.textContent = `${status.start}:00 — ${status.end}:00 today (Monday — Friday)`;
            const hasFixedOption = getWithdrawalOptionsForBalance(wBalance).length > 0;
            btn.disabled = !wPlan || !hasFixedOption;
            btn.classList.toggle('opacity-50', btn.disabled);
            btn.classList.toggle('pointer-events-none', btn.disabled);
        } else {
            banner.classList.remove('border-white/10', 'border-amber-500/20');
            banner.classList.add('border-red-500/20');
            title.textContent = 'Withdrawals Closed';
            title.className = 'text-sm font-bold text-red-400';
            desc.textContent = `Operating hours: ${status.start}:00 — ${status.end}:00, Monday — Friday`;
            btn.disabled = true;
            btn.classList.add('opacity-50', 'pointer-events-none');
        }
    }

    document.getElementById('wBankName').textContent = currentUser.bank_name || 'Not Linked';
    ...
    container.innerHTML = data.map(tx => {
        ...
        <div class="glass-panel rounded-2xl p-4 flex items-center justify-between border-white/5">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <div class="text-xs font-bold text-slate-300 uppercase tracking-widest font-display">${tx.type} ${tx.status === 'pending' ? '(Pending)' : ''}</div>
...
function switchAdminTab(tab) {
    const panels = ['users', 'deposits', 'withdrawals', 'giftcodes', 'settings'];
    panels.forEach(p => {
        ...
        const btn = document.getElementById('adminTab' + (p === 'deposits' ? 'Dep' : p === 'withdrawals' ? 'Wd' : p === 'giftcodes' ? 'Giftcodes' : p.charAt(0).toUpperCase() + p.slice(1)));
        if (btn) {
